...
2. BED files (feems_bed.fam, feems_bed.bim, feems_bed.bed)
Description: These are BED files generated by bcftools in filtering_pixy.sh, a necessary file format for several analyses (e.g., feems, conStruct).
...
4. Geographic location files (haust_locations.coord, haust_locations.outer)
...
1. Statistical analysis in R, including psi analysis (rangeExpansion) (analysis_plots.R)
...
2. Lines 6-12 of filtering_pixy.sh filter and format the VCF file. It also performs the PCA (VCF2PCACluster). These produce feems_filt.recode.vcf and the PCA values in revised_shovel_bug_results.txt. This also includes code for generating BED files from the filtered VCF file. 
...
4. Summary statistics - F, mean_depth, etc. - are obtained via vcftools (see https://vcftools.sourceforge.net/man_latest.html) and then added column by column into a single file (revised_shovel_bug_results.txt). 
5. The FEEMS analysis can now be performed on the BED files generated in step #2. Users can refer to https://github.com/NovembreLab/feems for installation instructions and required dependencies. Results can be formatted into csv files using the feems_script.py. NOTE: current script outputs files without the suffix "revise" as in the data folder.
...
14. plink v.1.9
15. R v.4.4.2
16. samtools (for tabix) v.1.22.1
17. bcftools v.1.22
